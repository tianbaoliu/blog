<template>
  <!-- 文件路径 node_modules/@vuepress-reco/vuepress-plugin-bgm-player/bin/BgMusic.vue -->
      ...
      <div class="reco-bgm-box" v-show="!isFloat" :style="panelPosition">
        ...
        <module-transition duration=".15">
          ...
          <div v-show="!isMini" class="reco-bgm-info">
            <!-- 歌曲名 艺术家名-->
            <div class="info-box">
              <span>{{ audio[curIndex].name }}</span>
              <span class="sp"> - {{ audio[curIndex].artist }} </span>
            </div>
            <!-- 歌曲进度条 -->
        ...
        <!-- 缩放按钮 -->
        <module-transition duration=".15">
          <div class="reco-bgm-left-box">
            <i
              v-show="!isMini"
              @click="changeBgmInfo(true)"
              class="reco-bgm reco-bgm-left"
            ></i>
            <i
              v-show="isMini"
              @click="changeBgmInfo(false)"
              class="reco-bgm reco-bgm-right"
            ></i>
          </div>
        </module-transition>
      </div>
